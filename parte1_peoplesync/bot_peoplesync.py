...
# Género: el formulario solo tiene Masculino / Femenino
GENERO_MAP = {
    "Masculino":           "Masculino",
    "Femenino":            "Femenino",
    "No binario":          "Femenino",
    "Prefiero no indicar": "Masculino",
}
...
# ── Setear campo date via JavaScript (necesario en Mac) ────────
def setear_fecha(driver, element_id, valor_yyyy_mm_dd):
    driver.execute_script(
        f"document.getElementById('{element_id}').value = '{valor_yyyy_mm_dd}';"
    )
...
def registrar_empleado(driver, wait, row):
    try:
        ...
        def fecha_js(element_id, valor_date):
            driver.execute_script(
        ...
        fecha_js("fecha_nacimiento", pd.to_datetime(row["fecha_nacimiento"]).strftime("%Y-%m-%d"))
        dropdown("genero", GENERO_MAP.get(str(row["genero"]).strip(), "Masculino"))
        ...
        fecha_js("fecha_ingreso", pd.to_datetime(row["fecha_ingreso"]).strftime("%Y-%m-%d"))
